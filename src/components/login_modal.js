...
import {fb_login,google_login} from '../store/action';
...
const SpringModal= (props)=> {
  ...
  const sell=(props)=>{
    console.log('S>S>S>S',props)
    if(props != null){
       History.push('./createadd')
    }else
    { handleOpen()}  } 
  return (
    <div>
        <button type="button" onClick={()=>sell(props.current_user.name)} style={{
            borderRadius:"30px",
      ...
      >
        <Fade in={open}>
          ...
          <button type="button" onClick={handleClose} className="c_btn">&times;</button>
            ...
            <button onClick={()=>props.fb_login(props.history)} className="px-sm-5  p-2 mx-sm-3 m-1 l_btn"><Icon className="pt-1">facebook</Icon>Continue with Facebook</button><br/>
            <button onClick={()=>props.google_login(props.history)} className="px-5 p-2 mx-sm-3 m-1 l_btn"><Icon className="pt-1">gamepadicon</Icon>Continue with Google</button><br/>
...
export default connect(mapStateToProps,mapDispatchToProp)(SpringModal);